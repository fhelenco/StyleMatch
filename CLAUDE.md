...
| `swapOutfitPiece()` | Text call — swap one piece in an existing outfit for a better wardrobe alternative | `claude-sonnet-4-6` |
| `rescoreOutfit()` | Text call — fixed item set → fresh cohesion score + notes (after the user manually adds a piece) | `claude-sonnet-4-6` |
...
- `claude-haiku-4-5-20251001` — cheapest, fastest (~2–4s per call), good enough for structured JSON extraction and outfit reasoning ($1/$5 per MTok). Used between early Sept and now; switched back to Sonnet for better nuance on busy patterns and ambiguous fabrics.
- `claude-sonnet-4-6` — current choice. Noticeably better nuance on busy patterns, ambiguous fabrics, and subtle style clashes than Haiku 4.5; ~2–3x slower and costlier ($3/$15 per MTok).
